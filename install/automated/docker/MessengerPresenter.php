...
final class MessengerPresenter extends OpenVKPresenter
{
    ...
    public function renderApiWriteMessage(int $sel): void
    {
        ...
        // Парсим вложения (как на стене)
        $attachments = [];
	error_log("MSG attachments param: " . var_export($this->postParam("attachments"), true));
	error_log("MSG content param: " . var_export($this->postParam("content"), true));
	if (!empty($this->postParam("attachments"))) {
	    ...
	    error_log("INPUT: " . implode(',', $attachmentIds));
	    error_log("OUTPUT count: " . count($attachments));
	    foreach($attachments as $a) { error_log("ATTACH: " . get_class($a) . " id=" . ($a ? $a->getId() : 'NULL')); }
        }

        // Разрешаем отправку, если есть текст ИЛИ есть валидные вложения
        if (empty($this->postParam("content")) && count($attachments) === 0) {
            header("HTTP/1.1 400 Bad Request");
            exit(json_encode(["error" => "Need text or attachments"]));
        }
